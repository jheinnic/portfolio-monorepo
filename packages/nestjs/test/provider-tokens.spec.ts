...
import { BindableProviderTokens } from '@jchptf/nestjs';
import { HasImpliedType, IsImpliedType } from '@jchptf/api';
import { A_SYMBOL, AnotherSubclass, Class, ISomething, MyGlobalModule, MyModule,
   OneSubclass, SomethingOne, SomethingThree, SomethingTwo } from './fixtures';
...
describe('ProviderTokens', () => {
   ...
   it('Maintains type information on tokens retrieved from a dictionary', () => {
      ...
      const fooToken: BindableProviderTokens<MyModule, Class> = A_SYMBOL;
      const barToken: BindableProviderTokens<MyGlobalModule, ISomething> = A_SYMBOL;

      type fooType = typeof fooToken;
      type barType = typeof barToken;
   ...
   it('Maintains equality when created equivalently', () => {
      // function blessIt<Component extends {}, Token extends IToken<MyModule>>(
        // token: IToken<MyModule>): LocalProviderToken<Component, MyModule, Token>
      // {
      //    return blessLocalProviderToken<Component, MyModule>(token, MyModule);
      // }
