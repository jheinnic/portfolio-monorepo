...
import {
   ...
   MerkleNodeType, MerkleOrientationType, IMerkleProofLocator, MerkleTreeDescription,
} from './locator';
import {
   BfsOrderOptions, DfsOrderOptions, IBfsOrderBuilder, IDfsOrderBuilder, IMerkleCalculator,
   IMerkleLocatorFactory,
} from './interface';
import {
 ...
 * traversing through a MerkleTreeEngine-managed merkle tree.
 *
 * The MerkleTreeEngine implements a mapping between the Merkle tree's logical structure as a
 * complete binary tree with relatively small records at each node, and a compact physical
 * arrangement that seeks to maintain locality of reference and minimize the number of random
 * access offsets required to perform a complete leaf-to-root path traversal without requiring an
 * excessive amount of cache storage during operation.
 *
 * It works by conceptually decomposing complete Merkle trees along layer boundaries spaced at
 * fixed intervals.  The result is a forest, with subtrees of equal height, with each subtree of a
 * given non-leaf layer related to exactly twice as many subtrees in the layer below as there are
 * leaves in its own structure.  The sole exception is made at the root node, which is permitted to
 * utilize a smaller depth than everywhere else such that the original tree's depth minus the root
 * overlay's actual depth is a perfect multiple of the depth use for every other decomposed
 * subtree.
 *
 * A complete binary tree of depth D has (2^D - 1) nodes.  Given an ideal I/O transfer size, R, and
 * output size H for hash digests, we know we can derive a density goal of R/H nodes per I/O.
 * MerkleTreeEngine's uses largest value for D such that (2^D - 1) <= R/H.  For example, given an
 * I/O transfer unit of 8K and a hash digest size of 256 bits:
 *
 * R / H = ((8192 bytes per transfer * 8 bits per byte) / (256 bits per hash)) = 256 hashes per
 * transfer
 *
 * ...such that...
 *
 ...
 *      D ~<= 8
 *
 * Given these sizing parameters, MerkleTreeEngine will map every 8 levels of hashes to a single
 * 8KB write. Imagine further that an instance of such a tree is to allocate storage for 2,000,000
 * entries.
 *
 * log2(2000000) =~ 20.932 <= 21
 *
 * It is not possible to divide partition 21 into some multiple of 8.  There are ways to split 16
 * or 24, but not 21.  The storage block closest to root is the sole exception that makes it
 * simpler to follow the convention throughout the remainder of any given Merkle tree.  It does,
 * however, burden MerkleTreeEngine with an obligation to track correlations between physical and
 * conceptual notions of place and partial ordering.  MerkleCalculator exists to hide the
 * implementation details and math behind MerkleTreeEngine's needs to map from the logical to the
 * physical and back again of the math required to map between the logical and physical addressing.
 *  The resulting engine code becomes a little more declarative, allowing its code to be read as
 * descriptions of the calculations it requires without having to describe how those calculation
 * requirements are to be met.
 *
 * Consumers and developers of this API are strongly encouraged to review the terminology that
 * follows. There are cases where the author has found it necessary to use synonymous terms to
 * describe concepts that are likewise similar except for the critically important few ways they
 * are different.  This leads to more concise method names in API design, but creates a necessity
 * to help consumers recognize when they need a glossary to resolve what otherwise should seem
 * ambiguous.
 *
 ...
 * -- "Depth" refers to the number of steps required by any proof constructed from a given Merkle
 * Tree, and Width refers to the maximum number of distinct items that can be spoken for using a
 * Merkle Tree of some "Depth".
 * -- "Height" is used to measure the distance between root and leaf in each block-mapped subtree
 * after the Merkle tree has been conceptually "split" for the sake of crafting I/O blocks.  Reach
 * is then used with one of two modifiers to count at the base of each subtree.  Reach is aways
 * defined in the context of a subtree's root node.
 *    -- "Internal Reach" refers to the number of nodes that are leaves still connected to the
 * contextual root.
 *    -- "External Reach" refers to the set of root nodes that were previously children of what are
 * now "leaf" nodes from the "Internal Reach"
 *    -- "Root" further modifies "Internal Reach" and "External Reach" to account for its unique
 * record count.
 *    -- The only time "Height" is modified is when we talk of "Root Height".
 *
 * -- "Level" uses the number of I/O transfers required along a path from a given leaf to the
 * tree's Merkle Root.  "Span" is a metric that changes from Level to Level.  It counts the number
 * of subtrees that were severed across all subtrees at a given layer.
 *
 ...
 *
 * The next set of terms are all ways of identifying a specific record using some measurement of
 * distance.
 *
 * -- Address is used to identify a distinct preImage from the source data set, but does do using
 * the corresponding location of its initial hash digest relative to the start of the Merkle Tree.
 * Because the entire Merkle Tree is not necessarily ever stored all in memory at once, this value
 * is not very useful for indexed access, but it is often used when forming or updating the
 * membership proof for an pathTo since its sequence of 0's and 1's will chart out the series of
 * left and and right traversal steps required to reach targeted digest
 *    -- The Address of a pre-image pathTo and the Offset of its Digest would be the same value,
 * but since the digests of a Merkle Tree are never really stored in sequential order, we instead
 * use the term "Address" to defined "that which we cannot have".
 * -- Offset is used to identify an indexed random access into physical storage and is 0-based from
 * the root node.
...
export class MerkleCalculator implements IMerkleCalculator
{
   ...
   /**
    * 2^D, where D is the depth (or height) of the subtree.  The layer of block roots above or
    * below any other block root node is always D layers away, and the number of internal nodes in
    * any block subtree will always be ((2^D) - 1).  There is a slight exception to this rule for
    * the deepest physical layer of a Merkle Tree that uses a number of layers that does not divide
    * evenly into the ideal subtree height for its block size to node size ratio.
    *
    * This number is frequently useful for calculations that have to deal with finding the
    * "absolute" logical offset range for all 2^D roots from the next lower physical layer whose
    * values are needed to calculate all 2^(D-1) leaf nodes of a block-mapped subtree from
    * "current" physical layer.
    *
    * For example, if a logical Merkle tree has been partitioned for mapping to physical blocks
    * along boundaries that split the overall tree into a forest of subtrees, each four layers
    * high, then there are 2^4 = 16 roots below each of the internal subtrees needed to calculate
    * each of its' own 2^3 = 8 leaf nodes.  A tree divided this way will define subtreeReach = 16,
    * and subtreeWidth = 8, and calculations will frequently retrieve one value or the other by
   ...
   /**
    * 2^(D-1), where D is the depth (or height) of the subtree.  The layer of logical leaf nodes at
    * the base of any given block-based subtree will always be D-1 layers away from their root.
    * There will always be 2^(D-1) such leaves, and the subtree itself will contain ((2^D) - 1)
    * nodes overall.
    *
    * This number is frequently useful for calculations that have to deal with finding the
    * "absolute" logical offset range for all 2^D roots from the next lower physical layer whose
   ...
   /**
    * To generalize this block partitioning strategy to apply generally to all potential Merkle
    * trees, it becomes necessary to concede that not every possible tree depth will require a
    * perfect multiple of the ideal subtree height for optimal block packing.  For 8192 byte
    * blocks, and 256 bit hash values, the optimal fit is a 7-layer subtree, with 127 internal
    * nodes and 128 leaves, consuming
    * 8160 out of every 8192 bytes.  By coincidence, the initial trees to be deployed with this
    * utility happen to be 21-layers tall, but they could easily grow to require 22-layers or
    * shrink to require only 20.  In fact, it is almost certain that the initial protocol will
    * periodically compact the merkle trees to remove space that is no longer required as records
    * are to be removed over time.
    *
    * To compensate for this reality, this library will remove overflow layers from the root node
    * such that the forest that remains after it has been removed is a perfect multiple that can
    * use optimal blocks for the rest of its span.  The purpose of this variable is to store the
    * actual depth to use instead of interlayerWidth for just the special case of the root node.
    */
   ...
   public* getLayers(topDown: boolean = true): IterableIterator<MerkleLayerLocator>
   {
      if (topDown) {
         ...
         for (let nextIndex = 0; nextIndex < maxIndex; nextIndex += 1) {
            yield this.locatorFactory.findLayerByDepth(nextIndex);
         }
      } else {
         for (let nextIndex = this.treeDescription.treeDepth - 1; nextIndex >= 0; nextIndex -= 1) {
            yield this.locatorFactory.findLayerByDepth(nextIndex);
         }
      }
   }
   ...
   public* getDigestsOnLayer(
      targetLayer: MerkleLayerLocator,
      leftToRight: boolean = true): IterableIterator<MerkleDigestLocator>
   {
   ...
   public* getRelatedDigestsOnLayer(
      fromRoot: MerkleDigestLocator, onLayer: MerkleLayerLocator, leftToRight: boolean = true)
   {
      if (onLayer.depth <= fromRoot.depth) {
         throw new Error(
            `Root layer, ${fromRoot.depth}, must be above iteration layer, ${onLayer.depth}`);
      }
      ...
      const stopIndex = leftToRight ? rightRelatedIndex + 1 : leftRelatedIndex - 1;
   ...
   public* getBlockMappedLayers(topDown: boolean = true): IterableIterator<BlockMappedLayerLocator>
   {
      if (topDown) {
         ...
         for (let nextIndex = this.treeDescription.tierCount - 1; nextIndex >= 0; nextIndex -= 1) {
            // yield this.locatorFactory.findBlockMappedLayerByLevel(nextIndex);
            yield this.locatorFactory.findLayerByDepth(
               this.blockMappedRootLayers[nextIndex])
               .asBlockMapped()
   ...
   public getTopoDigestOrder(director: IDirector<ITopoOrderBuilder>): Iterable<MerkleDigestLocator>
   {
      const options = TopoOrderOptions.create(director);
      return new MerkleTopologicalOrder(this, this.treeDescription, options);
   }

   public getTopoBlockOrder(
      director: IDirector<ITopoOrderBuilder>): BlockTopologicalOrder
   {
      const options = TopoOrderOptions.create(director);
      return new BlockTopologicalOrder(this, this.treeDescription, options);
   }

   public getBfsBlockOrder(
      director: IDirector<IBfsOrderBuilder>): Iterable<BlockMappedDigestLocator>
   {
   ...
   public getDfsBlockOrder(
      director: IDirector<IDfsOrderBuilder>): Iterable<BlockMappedDigestLocator>
   {
   ...
   public* getDigestPathToRoot(
      fromDigest: MerkleDigestLocator): IterableIterator<MerkleDigestLocator>
   {
   ...
   public* getChildBlockMappedRoots(
      parent: BlockMappedDigestLocator,
      leftToRight: boolean = true): IterableIterator<BlockMappedDigestLocator>
   {
      if (parent.blockLevel >= (
      ...
      leftToRight: boolean = true): IterableIterator<BlockMappedDigestLocator>
   {
      if (onLayer.level <= fromRoot.blockLevel) {
         throw new Error(
            `Root level, ${fromRoot.blockLevel}, must be above iteration layer, ${onLayer.level}`);
      }
      const layerSpan = onLayer.level - fromRoot.blockLevel;
      let distanceFactor = (
         fromRoot.blockOffset === 0
      ) ? this.rootSubtreeReach : this.subtreeReach;
      for (let ii = 1; ii < layerSpan; ii += 1) {
   ...
   public* getBlockMappedPathToRoot(
      fromDigest: MerkleDigestLocator): IterableIterator<BlockMappedDigestLocator>
   {
   ...
   public* getBlockMappedPathFromRoot(
      toDigest: MerkleDigestLocator): IterableIterator<BlockMappedDigestLocator>
   {
      const path = [...this.getBlockMappedPathToRoot(toDigest)];
      ...
      leftToRight: boolean = false): IterableIterator<MerkleDigestLocator>
   {
      ...
      while (currentLayer.depth !== stopLayerDepth) {
         ...
         currentDigest =
            this.locatorFactory.findDigestByLayerAndIndex(currentLayer, currentIndex);
   ...
   public* getSubtreesOnBlockMappedLayer(
      targetLayer: BlockMappedLayerLocator,
      leftToRight: boolean = true): IterableIterator<BlockMappedDigestLocator>
   {
      for (const nextDigest of this.getDigestsOnLayer(targetLayer, leftToRight)) {
         yield nextDigest.asBlockMapped()
            .get();
      }
   }
   ...
   public findLayerByDepth(depth: number): MerkleLayerLocator
   {
      if ((
         ...
         depth >= this.treeDescription.treeDepth
      ))
      {
         throw new Error(
   ...
   public findBlockLayerByLevel(level: number): BlockMappedLayerLocator
   {
      if ((
         ...
         level >= this.treeDescription.tierCount
      ))
      {
         throw new Error(
            `Layer depths must be between 0 and ${this.treeDescription.treeDepth
   ...
   public findParentBlockLayer(
      fromLayer: BlockMappedLayerLocator): Optional<BlockMappedLayerLocator>
   {
      if (fromLayer.depth === 0) {
         return Optional.empty();
      }
   ...
   {
      let levelIndex =
         binarySearch(this.blockMappedRootLayers, mapToStorage.depth, compareSorted);
      if (levelIndex < 0) {
         levelIndex = -1 * (
            levelIndex + 2
         );
   ...
   public findNearestBlockMappedRoot(digest: MerkleDigestLocator): BlockMappedDigestLocator
   {
      ...
      return currentDigest.asBlockMapped()
         .get();
   }

   public findParentBlockMappedRoot(
      fromChild: BlockMappedDigestLocator): Optional<BlockMappedDigestLocator>
   {
   ...
   public findMerkleProofForDigest(subjectDigest: MerkleDigestLocator): IMerkleProofLocator
   {
      if (subjectDigest.depth < (
         this.treeDescription.treeDepth - 1
      ))
      {
      ...
      const rootDigest = this.findDigestByPosition(0);
...
function labelOne(pathTo: BlockMappedSubtreeLocator)
{
   ...
   return `(${pathTo.level}::${pathTo.offset}<${pathTo.merkleSubtree.leftLeafPosition}
      to ${pathTo.merkleSubtree.rightLeafPosition}>)`;
}
*/
