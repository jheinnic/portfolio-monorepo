...
export class MerkleTreeDescription {
   ...
   public constructor(
      ...
      public readonly bitsPerIO: number,
      public readonly leafCapacity: number,
      public readonly leavesInUse: number)
   {
      if (bitsPerDigest > bitsPerIO) {
         throw Error(
            `Bits per digest, ${bitsPerDigest}, must not exceed bits per I/O, ${bitsPerIO}`);
      }
      if (leafCapacity < leavesInUse) {
         throw new Error(
            `Leaf use, ${leavesInUse}, may not exceed allocated capacity, ${leafCapacity}`);
      }

      this.treeDepth = Math.ceil(
         Math.log2(leafCapacity)) + 1;
      this.leafCapacity =
         Math.pow(2, this.treeDepth - 1);
      this.recordToDigestOffset =
         Math.pow(2, this.treeDepth - 1) - 1;

      this.subtreeDepth = Math.floor(
         Math.log2(bitsPerIO / bitsPerDigest),
      );
      ...
      const blockMappedRoots: number[] = [];
      blockMappedRoots.fill(-1, 0, this.tierCount - 1);
      const blockMappedLeaves: number[] = [];
      blockMappedLeaves.fill(-1, 0, this.tierCount - 1);
