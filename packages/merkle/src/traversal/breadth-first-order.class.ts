...
export class BreadthFirstOrder
{
      ...
      private readonly orderOptions: BfsOrderOptions)
   {
      ...
      if (this.layerSpan < 1) {
         throw new Error(
            `Leaf layer height, ${this.orderOptions.leafLayer!.blockHeight}, must be at least as deep as the root, ${this.orderOptions.rootBlock!.blockHeight}`);
      }
   }
   ...
   public* [Symbol.iterator](): IterableIterator<BlockMappedDigestLocator>
   {
      ...
      for (let ii = 1; ii < this.layerSpan; ii++) {
         currentLevel = this.calculator.findChildBlockLevel(currentLevel)
            .get();
