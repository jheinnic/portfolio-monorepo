declare module 'medium'
{
   ...
   export function chan<S = any, T = S>(
      numOrBuffer?: ChanBufferLike, xducer?: Transducer<S, T>): Chan<T>
    ...
    *
    * ports can be a channel to take from, a promise to resolve, or an array to put data onto a
    * channel, like [ theChannel, valueToPut ].
    *
    * If none of them have a pending value, it will resolve with whichever channel receives a value
    * next.
    *
   ...
   export function any<S1 = any, S2 = any, S3 = any>(
      port1: Alt<S1>, port2: Alt<S2>, port3: Alt<S3>): AnyAlt<S1 | S2 | S3>
   export function any<S1 = any, S2 = any, S3 = any, S4 = any>(
      port1: Alt<S1>, port2: Alt<S2>, port3: Alt<S3>, port4: Alt<S4>): AnyAlt<S1 | S2 | S3 | S4>
   export function any<S1 = any, S2 = any, S3 = any, S4 = any, S5 = any>(
      port1: Alt<S1>, port2: Alt<S2>, port3: Alt<S3>, port4: Alt<S4>,
      ...ports: Alt<S5>[]): AnyAlt<S1 | S2 | S3 | S4 | S5>

   /**
    * I don't love while loops, so I use this instead.
    *
    * As a bonus, you can track state without mutations! Return a value other than false, and it
    * will be available as the argument to your callback async function. Pass in a seed value as the
   ...
   /**
    * This is just like repeat above, except that before it repeats, it waits for a successful take
    * on the given channel. Then it passes this taken value in as the first argument, with any local
    * state being passed as the second argument.
    *
    * See the ping/pong example above to see this in action.
    */
   export function repeatTake<T = any, S = any>(
      ch: Chan<any, T>, fn: (cVal: T, iVal: S) => Promise<S | false>, seed: S): Promise<void>;
   export function repeatTake<T = any>(
      ch: Chan<any, T>, fn: (cVal: T) => Promise<void | false>): Promise<void>;

   /**
    * Creates a new channel that will receive all puts to the received channels.
    */
   export function merge<T1 = any, T2 = T1>(
      ch1: Chan<any, T1>, ch2: Chan<any, T2>): Chan<any, T1 | T2>;
   export function merge<T1 = any, T2 = T1, T3 = T2>(
      ch1: Chan<any, T1>, ch2: Chan<any, T2>, ch3: Chan<any, T3>): Chan<any, T1 | T2 | T3>;
   export function merge<T1 = any, T2 = T1, T3 = T2, T4 = T3>(
      ch1: Chan<any, T1>, ch2: Chan<any, T2>, ch3: Chan<any, T3>,
      ch4: Chan<any, T4>): Chan<any, T1 | T2 | T3 | T4>;
   export function merge<T1 = any, T2 = T1, T3 = T2, T4 = T3, T5 = T4>(
      ch1: Chan<any, T1>, ch2: Chan<any, T2>, ch3: Chan<any, T3>, ch4: Chan<any, T4>,
      ...chs: Chan<any, T5>[]): Chan<any, T1 | T2 | T3 | T4 | T5>;
}
