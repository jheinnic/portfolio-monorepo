...
type NgramHashRingData = {
    [key: string]: { weight: number }
}

export function seedWordsByHashRing(source: AsyncIterable<NgramListItem>, scale: number = 1): OperatorAsyncFunction<AsyncIterableX<number>, string> {
    ...
    reduce(from(source), {
        seed: { },
        callback: async (accum: NgramHashRingData, item: NgramListItem, _idx: number): Promise<NgramHashRingData> => {
            const weight: number = Math.ceil(item.count / scale);
            accum[item.ngram] = { weight };
            // accum[item.ngram] = { vnode };
            return accum;
        }
    }).then((ngramData: NgramHashRingData): void => {
        const hashRing = new HashRing(ngramData);
                ...
                return reduce(
                    sourceNums.pipe(
                        map(async (sourceBits: number): Promise<string> => {
                            return hashRing.get(sourceBits.toString(2));
                        })
                    ),
                    {
                        seed: "",
